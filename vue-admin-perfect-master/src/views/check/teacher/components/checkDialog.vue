  ...
  <el-dialog @close="close" v-model="dialogVisible" :title="title" width="50%">
    ...
    >
      <el-form-item label="审核" prop="state">
        <el-select v-model="ruleForm.state" class="m-2" placeholder="Select" size="large" clearable>
          <el-option  :key="0" label="待审核" :value="0"/>
          <el-option  :key="1" label="审核通过" :value="1"/>
          <el-option  :key="2" label="审核不通过" :value="2"/>
        </el-select>
      </el-form-item>
      <el-form-item v-if="ruleForm.state===2" label="未通过原因" prop="failreason">
...
<script lang="ts" setup>
  ...
  const rules = reactive({
    ...
    state: [{required: true, message: '请先审核', trigger: 'change',},],
  })
