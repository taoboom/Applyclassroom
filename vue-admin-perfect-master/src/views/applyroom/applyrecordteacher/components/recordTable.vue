      ...
      <div class="table-inner">
        <el-table
          ...
          <el-table-column prop="state" label="审核状态" align="center" width="100">
              <template #default="scope">
                <el-tag :type="scope.row.state2===1?'success':(scope.row.state2===2?'danger':'warning')"
                        >{{scope.row.state2===1?'审核通过':(scope.row.state2===2?'审核未通过':'待审核')}}</el-tag>
              </template>
          </el-table-column>
          ...
          <el-table-column prop="operator" label="操作" width="200px" align="center" fixed="right">
            <template #default="scope">
              <el-button v-if="scope.row.state2===0"
                         type="danger" size="small" icon="Delete"
                         @click="del(scope.row)"
              >取消申请</el-button>
              <el-tag v-else-if="scope.row.state2===1" type="success" >审核通过</el-tag>
              <template v-else>
